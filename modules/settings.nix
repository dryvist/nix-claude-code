...
let
  ...
  # the assertions below can compare, and return `null` for anything that
  # is not a bounded duration ("never", or a typo) so it fails the check
  # rather than silently meaning "wait forever".
    ...
    assertions = [
      ...
      }
      {
        ...
        message = ''
          ...
          An `ask` rule is evaluated before the auto-mode classifier and
          always forces a permission prompt, even in auto mode. In a
          scheduled run, CI job, container, or overnight session no human
          arrives to answer it, so the session stalls until timeout and the
          goal is abandoned — a prompt nobody can answer is an outage, not a
          control.

          Use programs.claude.settings.permissions.deny for a boundary that
          must never be crossed (it blocks without waiting on anyone), or
          programs.claude.autoMode.soft_deny / .hard_deny to teach the
          classifier about it.
        '';
      }
      {
        ...
        message = ''
          ...
          "${cfg.settings.askUserQuestionTimeout}", which is not a bounded
          duration of at most ${toString humanWaitCapSeconds}s.

          Use a "<number>s" or "<number>m" value up to "5m". "never" is
          rejected: an unanswered dialog would hold the session open
          indefinitely.
        '';
      }
      {
        assertion =
          cfg.settings.dialogExpiry == null || isWithinHumanWaitCap cfg.settings.dialogExpiry;
        message = ''
          programs.claude.settings.dialogExpiry is
          "${toString cfg.settings.dialogExpiry}", which is not a bounded
          duration of at most ${toString humanWaitCapSeconds}s.

          Use a "<number>s" or "<number>m" value up to "5m", or leave it
          null to keep Claude Code's own 5m default.
        '';
      }
    ];
  };
